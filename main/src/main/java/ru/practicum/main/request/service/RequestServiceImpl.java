...
public class RequestServiceImpl implements RequestService {
    ...
    public ParticipationRequestDto create(Long userId, Long eventId) {
        ...
        if (event.getInitiator().getId().equals(userId))
            throw new ConflictException("Нельзя отправить запрос на свое событие.");
        if (!event.getState().equals(Constant.State.PUBLISHED)) {
            throw new ConflictException("Нельзя отправить запрос на участие в неподтвержденном событии.");
        }
